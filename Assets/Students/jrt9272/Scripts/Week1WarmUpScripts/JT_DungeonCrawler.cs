...
public class JT_DungeonCrawler : MonoBehaviour {

    JT_DungeonLayout layout;

    [SerializeField]
    JT_CellVisuals[] visuals;
    [SerializeField]
    GameObject deadEnd;
    ...
    void Start() {
        //Get refs
        if (JT_DungeonLayout.instance != null) layout = JT_DungeonLayout.instance;
        audioSource = GetComponent<AudioSource>();

        //Initialize
        pos = layout.cells.Find(cell => cell.contents == JT_DungeonCell.CellContents.Start).coord;
    ...
    public void Movement(int dir) { 
        ...
        if (dir == 0) {
            JT_DungeonCell nextCell = layout.cells.Find(cell => cell.coord == new Vector2(pos.x + direction.x, pos.y + direction.y));
            //Check if player can walk forward
            if (nextCell.contents != JT_DungeonCell.CellContents.Empty) {
                //Update position and cell visuals
                pos += direction;
                StartCoroutine(CycleVisuals());
                //Check if the cell has an object, update UI and play appropriate SFX
                if (nextCell.contents == JT_DungeonCell.CellContents.Red) {
                   // ui.UpdateBlurb(2, direction);
                    audioSource.PlayOneShot(bumpSFX);
                }
                else if (nextCell.contents == JT_DungeonCell.CellContents.End) {
        ...
        if (dir == -2) {
            JT_DungeonCell nextCell = layout.cells.Find(cell => cell.coord == new Vector2(pos.x - direction.x, pos.y - direction.y));
            //Check if player can walk backward
            if (nextCell.contents != JT_DungeonCell.CellContents.Empty) {
                //Update position and cell visuals
                pos -= direction;
                StartCoroutine(CycleVisuals());
    ...
    IEnumerator CycleVisuals() {
        ...
        foreach (JT_CellVisuals vis in visuals) {
            ...
            if (!blockedView) {
                ...
                JT_DungeonCell nextCell = layout.cells.Find(cell => cell.coord == new Vector2(pos.x + direction.x * i, pos.y + direction.y * i));
                if (nextCell != null && nextCell.contents != JT_DungeonCell.CellContents.Empty) { // if there is something to be drawn                 
                    if (i == 1) deadEnd.SetActive(false); //Override
                    //Check if there is an object in this cell
                    if (nextCell != null && nextCell.contents == JT_DungeonCell.CellContents.End) {
                        toggles[4] = true;
                        colorIndex = 0;
                    } else if (nextCell != null && nextCell.contents == JT_DungeonCell.CellContents.Red) {
                        toggles[4] = true;
                        colorIndex = 1;
                    }
                    //Check for open hallways in adjacent cells
                    if (direction.x != 0) { //Check north south 
                        JT_DungeonCell adjCell = layout.cells.Find(cell => cell.coord == new Vector2(nextCell.coord.x, nextCell.coord.y + direction.x));
                        if (adjCell.contents != JT_DungeonCell.CellContents.Empty) {
                            toggles[1] = true;
                        }
                        ...
                        JT_DungeonCell adjCell = layout.cells.Find(cell => cell.coord == new Vector2(nextCell.coord.x - direction.y, nextCell.coord.y));
                        if (adjCell.contents != JT_DungeonCell.CellContents.Empty) {
                            toggles[1] = true;
                        }
                    ...
                    if (nextCell.contents == JT_DungeonCell.CellContents.Empty) { // check if closed
                        toggles[3] = true;
                    }
